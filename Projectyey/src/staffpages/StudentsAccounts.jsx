...
function StudentAccounts() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('https://projectyey.vercel.app/student/nonArchivedStudents');
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get(`https://projectyey.vercel.app/student/searchStudents?keyword=${searchTerm}`);
      if (response.status === 200) {
        setData(response.data);
        console.log('Student accounts fetched successfully:', response.data);
      } else {
  ...
  const handleArchiveConfirm = async () => {
    try {
      ...
      const response = await axios.post(`https://projectyey.vercel.app/student/archive/${selectedUser.id}`);
  ...
  const handleUpdate = async () => {
    ...
    if (selectedUser && selectedUser.id) {
      try {
        ...
        const response = await axios.put(`https://projectyey.vercel.app/students/updateStudent/${selectedUser.id}`, selectedUser);
        if (response.status === 200) {
