...
function DoctorAccounts() {
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get(`https://dentalmanagement-app.onrender.com/doctor/searchDoctor?keyword=${searchTerm}`);
      if (response.status === 200) {
        setData(response.data);
  ...
  const handleArchiveConfirm = async () => {
    try {
      ...
      const response = await axios.post(`https://dentalmanagement-app.onrender.com/doctor/archiveDoctor/${selectedUser.id}`);
  ...
  const handleUpdate = async () => {
    ...
    if (selectedUser && selectedUser.id) {
      try {
        ...
        const response = await axios.put(`https://dentalmanagement-app.onrender.com/doctor/updateDoctor/${selectedUser.id}`, selectedUser);
        if (response.status === 200) {
