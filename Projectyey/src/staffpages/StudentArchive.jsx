...
function StudentArchive() {
  ...
  const fetchArchivedAccounts = async () => {
    try {
      const response = await axios.get('https://dentalmanagement-app.onrender.com/student/archivedStudents');
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get(`https://dentalmanagement-app.onrender.com/student/search/archivedStudents?keyword=${searchTerm}`);
      if (response.status === 200) {
        setArchivedAccounts(response.data);
  ...
  const handleUnarchive = async (id) => {
    try {
      const response = await axios.post(`https://dentalmanagement-app.onrender.com/student/unarchiveStudent/${id}`);
      if (response.status === 200) {
