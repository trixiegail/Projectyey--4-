...
function StaffArchive() {
    ...
    const fetchArchivedAccounts = async () => {
        try {
            const response = await axios.get('https://projectyey.vercel.app/staff/getAllArchivedStaffs');
    ...
    const handleSearch = async () => {
        try {
            const response = await axios.get(`https://projectyey.vercel.app/staff/search/archivedStaffs?keyword=${searchTerm}`);
            if (response.status === 200) {
                setArchivedAccounts(response.data);
    ...
    const handleUnarchive = async (id) => {
        try {
            const response = await axios.post(`https://projectyey.vercel.app/staff/unarchiveStaff/${id}`);
            if (response.status === 200) {
    ...
    return (
        <div className='ml-[265px]'>
            ...
            <div className="w-50 ml-10 mt-[120px] relative">
                <h1 className="text-2xl font-bold mb-5">Archived Staff Accounts</h1>
