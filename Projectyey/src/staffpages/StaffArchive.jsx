...
function StaffArchive() {
    ...
    const fetchArchivedAccounts = async () => {
      try {
        const response = await axios.get('http://localhost:8080/user/staffs/archived');
    ...
    const handleSearch = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/user/staffs/search/archived?keyword=${searchTerm}`);
        if (response.status === 200) {
          setArchivedAccounts(response.data);
    ...
    const handleUnarchive = async (id) => {
      try {
        const response = await axios.post(`http://localhost:8080/user/staffs/unarchiveStaff/${id}`);
        if (response.status === 200) {
