...
function StaffArchive() {
    ...
    const fetchArchivedAccounts = async () => {
        try {
            const response = await axios.get('http://localhost:8080/staff/getAllArchivedStaffs');
    ...
    const handleSearch = async () => {
        try {
            const response = await axios.get(`http://localhost:8080/staff/search/archivedStaffs?keyword=${searchTerm}`);
            if (response.status === 200) {
                setArchivedAccounts(response.data);
    ...
    const handleUnarchive = async (id) => {
        try {
            const response = await axios.post(`http://localhost:8080/staff/unarchiveStaff/${id}`);
            if (response.status === 200) {
